...
def main() -> int:
    ...
    parser.add_argument("--em-dash", type=int, default=None, metavar="N", help="Override the em dash limit for this run (paper default 3); does not alter the shipped default")
    args = parser.parse_args()

    try:
        result = inspect(_read_text(args.path), args.scope)
        if args.em_dash is not None:
            result["em_dash_limit"] = args.em_dash
            result["issues"] = [i for i in result["issues"] if i["code"] != "em_dash_limit" or result["em_dash_count"] > args.em_dash]
