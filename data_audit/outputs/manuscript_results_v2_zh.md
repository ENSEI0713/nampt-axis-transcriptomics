...
**轴内免疫负荷（新）。** 在 GSE318937（n=118）与 GSE305038 中，数据集内全样本的 inflammatory_score 与轴内单核/巨噬基因负荷高度相关（r=0.77–0.89，落在 0.7–0.9 范围），独立印证 Phase 1b 敏感性中"急性炎症样信号主要由 NF-κB/细胞因子模块驱动"的结论。该急性信号在 24 h 后回落（GSE318937 即时 vs 24h balance 差异）。注：相关计算基于数据集内全样本，未按运动后时间点子集拆分。
