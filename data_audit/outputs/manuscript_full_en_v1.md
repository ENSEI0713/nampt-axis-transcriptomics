...
**eQTL/GWAS merge (complete).** GTEx REST (4 relevant tissues) + GWAS Catalog per-rsid queries for all 432 candidates (Supplementary Data 8). Distribution: **A = 0, B = 0, C = 97, External-only = 9, Excluded = 326**. Tier A/B empty is an honest result: the high-perturbation shortlist is almost entirely rare variants, GTEx covers only common variants, and GWAS Catalog coverage of these rsids is sparse, so Evo2 perturbation and externally reproducible support barely overlap; the 9 External-only variants are strongly supported common loci (TNF rs1800629, IL6 rs1800795, etc.) with mild Evo2 scores.
