...
Obesity is commonly accompanied by low-grade inflammation, insulin resistance and reduced mitochondrial function, and the challenge of exercise in this setting is to distinguish a recoverable adaptive stress from additional load superimposed on existing inflammation. The NAMPT axis links NAD salvage and cellular repair to immune and metabolic stress, yet the state dependence of NAMPT-axis transcriptional programs — whether they behave as inflammation-like or repair-like programs across tissue, time and metabolic background — remains unclear. We constructed a 59-gene, two-program NAMPT-axis score (repair and inflammatory), applied it within 9 public human transcriptomic units (337 samples) spanning skeletal muscle, CD14+ monocytes, visceral adipose tissue and a human muscle-cell model, and tested the framework with module-level internal consistency (Cronbach α), predefined contrasts, random-effects meta-analysis with cluster-aware and leave-one-dataset-out sensitivity, and a sensitivity series (Phase 1b). Results: (i) the two-program structure is supported by cross-dataset co-expression in large cohorts (repair-module α = 0.79–0.91); (ii) acute exercise raises NAMPT_z reproducibly (contrast-level random-effects meta: +0.85, 95% CI 0.53–1.17, 92% direction agreement, p ≈ 1.8e-7; dataset-level k = 4, +0.965, p ≈ 2.8e-4), while repair_score does not rise significantly (−0.33, p = 0.05) and balance_score loses significance at cluster-aware precision (k = 4, −0.47, p = 0.24) — NAMPT upregulation is not automatically repair, and a directional shift of the balance is weak evidence; (iii) the same NAMPT upregulation couples to a stronger inflammatory program under lipotoxic stimulation (cell-model meta: inflammatory_score +0.55, 100% direction agreement) and to a distinct transcriptomic context under AICAR exercise-mimetic stimulation; these results do not constitute evidence of NAD-metabolite change. As an exploratory module, Evo2-40B scored 432 NAMPT-axis regulatory candidate variants against public eQTL/GWAS evidence; because the high-perturbation shortlist is dominated by rare variants, Tier A/B are empty and the module is reported as supplementary with a verifiable candidate list. We conclude that the NAMPT-NAD system is not captured by a single directional marker: its transcriptional state must be interpreted across time, training state and metabolic context. Public data support this model, but the evidence is transcriptomic only; eNAMPT protein/NAD metabolites and causality remain unverified predictions.
...
**Score A (allele surprisal, complete).** All 432 candidates scored (flank 200 bp, Evo2-40B generate logits). |delta| median 1.12; |delta| ≥ 4: 94 variants. High perturbation concentrates in IL6 / SIRT1 / TNF windows (observation, not causal).
...
- **Score B (pseudo-likelihood, shortlist complete)**: 104 shortlist (94 |delta| ≥ 4 + 12 NAMPT full-window), downstream 8 bp two-strand propagation: `PLL = Σ_i log P(base_i | context + allele + bases<i>)`, `delta_PLL = PLL(alt) − PLL(ref)`; 32 generate calls per variant, 0 failures (`scores_B.csv`). delta_PLL median −0.876; Score A↔B sign agreement 87%; **strand consistency 65% (68/104), fwd/rc r ≈ 0.13** — limited double-strand convergence; Tier C treated as hypothesis set, not stable-ranked list (R1-M7);
...
## Discussion (brief working skeleton)

The manuscript currently has Results and Methods fully drafted (Chinese working draft → English v1). A full Discussion section has not yet been written; this is the natural next step before final English consolidation. Intended content (from `manuscript_logic_v1_zh.md`):
1. The state-dependent NAMPT-axis model resolves the "is NAMPT good or bad" tension: the same NAMPT upregulation carries different program direction under exercise vs lipotoxicity;
2. Methodological honesty as strength: cluster-aware meta, Tier A/B = 0, Evo2 as supplementary — negative/uncertain results reported rather than packaged;
3. Evidence boundaries: transcriptomic only; eNAMPT protein/NAD metabolites as testable predictions; small samples directional; public data not for individual prescription;
4. Limitations: no eNAMPT/NAD measurement, small obesity-layer k, strand-consistency limit of Evo2, no cross-study moderator meta-regression;
5. Next steps: MoTrPAC / plasma eNAMPT-NAD metabolite validation, MPRA/CRISPRi on Tier C candidates, prospective exercise cohort.

---

*End of EN draft v1. Numbers and boundaries to be re-verified against the source table before journal submission; Discussion to be completed; references to be compiled (Step 4).*
